...
def _unwrap_optimized_module(model: torch.nn.Module) -> torch.nn.Module:
    """Use the underlying module for state-dict IO when torch._dynamo wraps the model."""
    return getattr(model, "_orig_mod", model)


def adapt_patch_embed_input_channels(state_dict: dict, model: torch.nn.Module) -> dict:
    patch_embed_key = "patch_embed.proj.weight"
    if patch_embed_key not in state_dict:
        return state_dict

    target_weight = model.patch_embed.proj.weight
    source_weight = state_dict[patch_embed_key]
    target_in_chans = target_weight.shape[1]
    source_in_chans = source_weight.shape[1]
    if source_in_chans == target_in_chans:
        return state_dict

    state_dict = dict(state_dict)
    if source_in_chans == 1 and target_in_chans == 3:
        logger.info(
            "Adapting %s from grayscale to RGB by repeating and averaging channels",
            patch_embed_key,
        )
        state_dict[patch_embed_key] = source_weight.repeat(1, target_in_chans, 1, 1) / target_in_chans
        return state_dict

    if source_in_chans == 3 and target_in_chans == 1:
        logger.info(
            "Adapting %s from RGB to grayscale by channel averaging",
            patch_embed_key,
        )
        state_dict[patch_embed_key] = source_weight.mean(dim=1, keepdim=True)
        return state_dict

    raise ValueError(
        f"Unsupported patch embed channel conversion {source_in_chans} -> {target_in_chans}"
    )
...
def load_checkpoint(
    ...
    **others: Stateful,
    ...
    """
    ckpt_dir = Path(ckpt_dir)
    state_dict_model = _unwrap_optimized_module(model)
    to_load = {"iteration": None}
    to_load["model"] = dcpsd.get_model_state_dict(state_dict_model)
    if optimizer is not None:
        to_load["optimizer"] = dcpsd.get_optimizer_state_dict(state_dict_model, optimizer)
    ...
    dcpsd.set_model_state_dict(state_dict_model, to_load["model"])
    if optimizer is not None:
        dcpsd.set_optimizer_state_dict(state_dict_model, optimizer, to_load["optimizer"])
...
def init_fsdp_model_from_checkpoint(
    ...
    if not Path(checkpoint_path).is_dir():  # PyTorch standard checkpoint
        ...
        try:
            chkpt = torch.load(checkpoint_path, map_location="cpu", weights_only=True)["teacher"]
        except TypeError:
...
def init_model_from_checkpoint_for_evals(
    ...
    state_dict = adapt_patch_embed_input_channels(state_dict, model)
