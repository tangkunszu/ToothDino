...
def _parse_dataset_root(dataset_str: str) -> str | None:
...
def _prepare_representative_tooth_cache(cfg, dataset_path: str):
    ...
    dataset_root = _parse_dataset_root(dataset_path)
    if not dataset_root:
        logger.warning("Cannot auto-build representative_tooth_cache: dataset root not found in %s", dataset_path)
        return

    if not cache_path:
        root_path = Path(dataset_root).resolve()
        root_slug = root_path.name or "dataset"
        root_hash = hashlib.sha1(str(root_path).encode("utf-8")).hexdigest()[:8]
        cache_path = Path("cache") / f"representative_tooth_centers_{root_slug}_{root_hash}_adaptive.json"
        with open_dict(crops_cfg):
            crops_cfg.representative_tooth_cache_path = str(cache_path)
        logger.info("representative_tooth_cache_path not set; using auto path: %s", cache_path)

    cache_path = Path(cache_path)
    if cache_path.is_file():
        logger.info("representative_tooth_cache found: %s", cache_path)
        return

    is_main_process = distributed.is_main_process()
    if is_main_process:
        logger.info("representative_tooth_cache missing, building: %s", cache_path)
        try:
            from tools.precompute_representative_tooth_cache import build_representative_tooth_cache

            workers = min(max(int(getattr(cfg.train, "num_workers", 1)), 1), 16)
            build_representative_tooth_cache(
                data_dir=dataset_root,
                out=str(cache_path),
                mode=crops_cfg.get("representative_tooth_cache_mode", "adaptive"),
                workers=workers,
                global_crops_size=int(crops_cfg.global_crops_size),
                local_crops_size=int(crops_cfg.local_crops_size),
                local_crops_number=int(crops_cfg.local_crops_number),
                progress=True,
            )
            logger.info("representative_tooth_cache built: %s", cache_path)
        except Exception:
            logger.exception(
                "Failed to build representative_tooth_cache at %s; falling back to online dynamic centers",
                cache_path,
            )
            with open_dict(crops_cfg):
                crops_cfg.representative_tooth_cache_path = None
    ...
    if not cache_path.is_file():
        if not is_main_process:
            logger.warning(
                "representative_tooth_cache unavailable after waiting at %s; falling back to online dynamic centers",
                cache_path,
            )
            with open_dict(crops_cfg):
                crops_cfg.representative_tooth_cache_path = None
    ...
    logger.info("representative_tooth_cache ready: %s", cache_path)


def keep_last_n_eval_checkpoints(eval_root: Path | str, n: int | None):
    """Keep only the latest N eval snapshots named training_<iter>."""
    if n is None:
        return
    eval_root = Path(eval_root)
    if not eval_root.is_dir():
        return
    pattern = re.compile(r"training_(\d+)$")
    checkpoints = []
    ...
    for _, ckpt_dir in checkpoints[:-n]:
        try:
            shutil.rmtree(ckpt_dir)
            logger.info("Deleted old eval checkpoint: %s", ckpt_dir)
        except Exception:
            logger.exception("Failed to delete old eval checkpoint: %s", ckpt_dir)
...
For python-based LazyConfig, use "path.key=value".
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to save logs and checkpoints. If omitted, use train.output_dir from the config.",
    )
...
def do_test(cfg, model, iteration, process_group, do_low_freq=False):
    ...
    eval_root = Path(cfg.train.output_dir) / "eval"

    def save_sharded_teacher_checkpoint():
        ...
        if distributed.is_subgroup_main_process():
            keep_last_n_eval_checkpoints(eval_root, getattr(cfg.evaluation, "eval_checkpoint_max_to_keep", 1))
...
def build_data_loader_from_cfg(
    ...
    mask_generator = MaskingGenerator(
        ...
        bias_mode=cfg.masking.bias_mode,
        bias_strength=cfg.masking.bias_strength,
        focus_center_x=cfg.masking.focus_center_x,
        focus_center_y=cfg.masking.focus_center_y,
        focus_spread_x=cfg.masking.focus_spread_x,
        focus_spread_y=cfg.masking.focus_spread_y,
    )
    ...
    _prepare_representative_tooth_cache(cfg, dataset_path)
...
def do_train(cfg, model, resume=False):
    ...
    for data in metric_logger.log_every(
        ...
        #新添加的
         # +RAM ---- 每 256 it 强制回收 + 看 RSS ----
        if (iteration & 512) == 0:               # 256 可调
            gc.collect()                         # 1. 收 Python 垃圾
            # 2. 把 Python 申请的多余空间还给操作系统
            if hasattr(gc, 'shrink'):
                gc.shrink()                    
    if (
        cfg.evaluation.eval_period_iterations > 0
        and iteration > start_iter
        and iteration % cfg.evaluation.eval_period_iterations != 0
    ):
        final_eval_iteration = iteration - 1
        logger.info("Running final eval checkpoint save at iteration %d", final_eval_iteration)
        do_test(cfg, model, f"training_{final_eval_iteration}", process_group=process_subgroup)
        gc.collect()
        if hasattr(gc, 'shrink'):
            gc.shrink()
        torch.cuda.synchronize()
...
def main(argv=None):
    ...
    if args.multi_distillation:
        ...
        # Log configs safely (avoid formatting errors with DictConfig)
        logger.info("%s", cfg)
        resolved_output_dir = args.output_dir or cfg.train.output_dir
        setup_logging(output=os.path.abspath(resolved_output_dir), level=logging.INFO)
        setup_logging(
            output=os.path.join(os.path.abspath(resolved_output_dir), "nan_logs"),
            name="nan_logger",
            level=logging.INFO,
        )
